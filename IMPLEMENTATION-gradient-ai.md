...
- [x] **LIVE** (real token, DECISIONS D24/D25): provisioned BOTH KBs on the real account (permit KB `7bf2446c-…`, **menu KB `817bb6d1-…`**); live competition-overlap ran against the real menu KB (`provider=gradient kb_verified=true`, taqueria=0.7/coffee=0, item+price not cuisine); runtime live on serverless inference (`key_present:true`, `llama3.3-70b-instruct`); **`run.mjs --live` GREEN 12/12**. KB doc ingestion needs a Spaces bucket on this account (HTTP 400) so the menu is mirrored locally per D19; endpoint latency was high so the live pass is structural with deterministic checklist correctness (D25).
