...
- **Decision:** since managed Agent creation is blocked but inference is fully available, I built a
  ...
  - **Latency (cut ~10x):** the checklist is **loaded from the authored `kb/<vendor_type>.md`**, not
    regenerated by the model, so Permit only writes short prose → **permit checklist ~7–10s**
    (was ~85–95s), permit Q&A ~7–15s, Spot Scout ~15–20s, jailbreak instant. Deterministic
    citations (checklist cites + keyword-retrieved rule docs) also make the fire-permit answer cite
    `sffd-permit` reliably. Managed Agents (server-side RAG) remain the production path once enabled.
