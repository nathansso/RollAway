...
Measured live on Llama 3.3 70B:
- **Permit checklist ~7–10s.** The checklist is **loaded from the authored `kb/<vendor_type>.md`**
  (never regenerated by the model), so the model only writes the short prose reply. This is the
  big win — it was ~85–95s when the model regenerated the whole JSON, and it can't drift from the
  source of truth. Citations are built deterministically from the checklist's cites plus any rule
  doc the question mentions (e.g. a "fire permit?" question pulls in `sffd-permit`).
- **Permit Q&A ~7–15s** (no vendor type → grounded model answer, lean/retrieved KB context).
- **Spot Scout ~15–20s** (function-calling loop over the tools); **jailbreak instant** (refused
...
- **Model reliability:** occasionally the model omits an envelope field; the runtime backfills
  clearance citations from the tool output, loads the checklist deterministically, and coerces the
  envelope shape; the §A validator flags anything left (visible via `?debug=1`). A stronger model
  or the managed RAG runtime improves prose quality.
